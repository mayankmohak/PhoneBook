...
def Database():
    ...
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS `member` (mem_id INTEGER NOT NULL  PRIMARY KEY AUTOINCREMENT, firstname TEXT, lastname TEXT, gender TEXT, email TEXT, address TEXT, contact TEXT)")
    cursor.execute("SELECT * FROM `member` ORDER BY `firstname` DESC")
...
def SubmitData():
    if  FIRSTNAME.get() == "" or LASTNAME.get() == "" or GENDER.get() == "" or EMAIL.get() == "" or ADDRESS.get() == "" or CONTACT.get() == "":
        ...
        if not checkname(fname):
            ...
            tkMessageBox.showwarning('', 'EMAIL can be a valid email only', icon="warning")
        elif not checkcontact(cont):
            tkMessageBox.showwarning('', 'Ph Number 10 digits only', icon="warning")
        else:
            cursor.execute("INSERT INTO `member` (firstname, lastname, gender, email, address, contact) VALUES(?, ?, ?, ?, ?, ?)", (fname, lname, gen, email, addr, cont))
            conn.commit()
            FIRSTNAME.set("")
            LASTNAME.set("")
            GENDER.set("-")
...
def UpdateData():
    if GENDER.get() == "":
        ...
        if not checkname(fname):
            ...
            cursor.execute("UPDATE `member` SET `firstname` = ?, `lastname` = ?, `gender` =?, `email` = ?,  `address` = ?, `contact` = ? WHERE `mem_id` = ?", (fname, lname, gen, email, addr, cont, int(mem_id)))
            conn.commit()
            FIRSTNAME.set("")
            LASTNAME.set("")
            GENDER.set("-")
...
def OnSelected(event):
    ...
    lbl_email = Label(ContactForm, text="Email", font=('Kristen ITC', 14), bd=5)
...
#============================LABELS======================================
lbl_title = Label(Top, text="Phone Book", font=('Cooper Black', 58), width=500)
lbl_title.pack(fill=X)

#============================ENTRY=======================================

#============================BUTTONS=====================================
btn_add = Button(MidLeft, text="ADD NEW", font=('Kristen ITC',10), bg="#66ff66", command=AddNewWindow)
btn_add.pack()
...
tree = ttk.Treeview(TableMargin, columns=("MemberID", "Firstname", "Lastname", "Gender", "Email", "Address", "Contact"), height=400, selectmode="extended", yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
